				...
				if($newUser != ''){
					...
					socket.emit('new user',$newUser,function(data){
						if(data){
							$chatWrap.slideDown();
							$loginWrap.slideUp();
							$('#title').html('Bienvenue sur Connectez-vous '+$newUser);
							$username.val('');
						}else{
							$('#loginMsg').html('<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> <span class="sr-only">Error:</span>Ce nom exite déja, choisissez un autre nom</div>');
						}
					});
				}else{
						$('#loginMsg').html('<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> <span class="sr-only">Error:</span>Veuillez s\'identifier s\'il vous plait</div>');
						$(this).disabled = false;
				}
			});
...
$('#connect').on('click',function(){
	...
	if(!isStarted){
		...
		if(remoteUser != ''&&remoteUser != self){
			...
			alert('Enter le nom de votre correspondant s\'il vous plait');
	}else{
			$('#callStatus').html('Appel en progression');
	}
});
